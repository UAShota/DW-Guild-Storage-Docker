...
class DwgbCmdButtons(DwgbCmdCustom):
    ...
    def showButtonsBaf(self, message: DwgbMessage):
        """ Показ набора кнопок """
        return self.send(message, "Ойбай кнопки", '{"one_time":true,"buttons":[[{"action":{"type":"text","label":"Апо 1","payload":""},"color":"secondary"},{"action":{"type":"text","label":"Апо 2","payload":""},"color":"secondary"},{"action":{"type":"text","label":"Апо 3","payload":""},"color":"secondary"},{"action":{"type":"text","label":"Апо 4","payload":""},"color":"secondary"}]]}')
